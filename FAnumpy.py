...
from mom import load_data_digits


def InitNN(num_inputs, num_hiddens, num_outputs):
    """Initializes NN parameters.
    Args:      num_inputs   :   Number of input units =image height x image width
               num_hiddens  :   List of hidden size for each layer [16,32].
               num_outputs  :   Number of output units.
    Returns:   model        :   Randomly initialized network weights.
    """
...
def AffineBackward(grad_y, x, w):
    ...
    Arg: grad_y : gradient from last layer, x : inputs, w : weights
    Returns:    grad_x, grad_w, grad_b: Gradients wrt
                the inputs, the weights, the biases
    """
...
def activate_backward(grad_y, x, y, function):
...
def ReLU(x):
    """Computes the ReLU activation function with x: Inputs"""
...
def activation(activation_function, x):
...
def Train(model, forward, backward, update, eps, momentum, num_epochs,
    ...
    for epoch in range(num_epochs):
        ...
        if epoch == num_epochs:
            for i in range(len(prediction)):
                if np.sort(prediction[i])[0, 6] - np.sort(prediction[i])[0, 5] < 0.3:
    ...
    print('AC: Train {:.5f} Validation {:.5f} Test {:.5f}'.format(
